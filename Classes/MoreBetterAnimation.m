...
- (MoreBetterAnimations *)init { 
	if (self = [super init]) { 
		...
		_repeatCounter = 0; 
		_timeElapsed = 0; 
		_displayLink= [CADisplayLink displayLinkWithTarget:self selector:@selector(changeAnimationImage) ]; 
		return self;
	}
}
...
-(void)changeAnimationImage { 
	...
	if ( (_timeElapsed >= self.animationDuration || _frameCounter >= self.animationImages.length) && (0 < self.animationRepeatCount && _repeatCounter <= self.animationRepeatCount) ) { 
		_repeatCounter++;
		_frameCounter = 0;
	} 
